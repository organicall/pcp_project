// pagerank_paper1_improved_3_local.cpp
//
// identical to pagerank_paper1_improved_3.cpp in every way except the accumulator
// is forced to the thread-local flat array path regardless of memory footprint.
//
// used in test 3 to isolate the cost of the local-array strategy at high thread
// counts where the footprint exceeds l3. the adaptive version would switch to
// atomics in that regime, so without this forced variant you can't see how much
// the local arrays degrade when they spill out of cache.
...
int main(int argc, char* argv[]) {
    ...
    // forced local: always allocate nthreads*n doubles for the local array even if
    // it exceeds l3. this lets us observe how much degradation occurs at high thread
    // counts where the arrays no longer fit in cache.
    long long localAccBytes = (long long)nthreads * n * sizeof(double);
    bool useLocalAcc = true;

    std::vector<double> localAcc; // always used — forced local mode
    std::vector<double> acc;      // unused in this variant

    localAcc.assign((long long)nthreads * n, 0.0);
    std::cout << "Accumulator: forced-localAcc (mode 1 always)\n";
    ...
    for (int iter = 0; iter < MAX_ITER; iter++) {
        ...
        if (useLocalAcc) {
            ...
            // this branch is never taken in the forced-local variant
            #pragma omp parallel for schedule(guided) num_threads(nthreads)
            for (int p = 0; p < k; p++) {
                for (auto& e : parts[p].edgeList) {
                    #pragma omp atomic
        ...
        // gather: each vertex reads one slot per thread, sums them, and clears them.
        // at high thread counts where localAcc spills out of l3 these reads become
        // expensive dram accesses — that's exactly the degradation we want to measure.
        double err = 0.0;
        if (useLocalAcc) {
